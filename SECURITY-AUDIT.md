...
| D-KAFKA-03 | measured | MED | A topic whose partition 0 is EMPTY is reported as non-existent while its records sit unreachable. `topicPartitionCount` counts from partition 0 and stops at the first empty one, so a topic with data only on, say, partition 6 answers 0 partitions. The Java producer's sticky partitioner puts a small run entirely on ONE partition, so the sparse layout is ordinary, not exotic — only reached when the topic is absent from the registry | Needs an internal CROSS-SHARD scan, which does not exist: `gKafkaExec` routes a command by `args[1]`, so a `KEYS` sent from the skin lands on the one shard whose hash matches the PATTERN and sees only that shard's keyspace (the client-facing `KEYS` does span shards — verified, 8 partitions across 4). Batching one variadic `EXISTS` per owning shard fixed the single-shard case (a consumer went from 0 to 800 records) but not the sharded one, and `kafka-topics.sh --describe` still denies the topic even then — so a second, unidentified gate exists outside `topicPartitionCount`. Both attempts reverted rather than shipped half | repro in the row below |
| D-KAFKA-03r | — | — | **Reproduction**: start with `--kafka-db 5` (puts the Kafka keyspace in a SELECT-able db), create a topic with 8 partitions, produce 800 records with `kafka-producer-perf-test`, `DEL` the empty partition lists, `HDEL kafka.topics <topic>`, then `--describe` and consume | — | — |
