...
| D-MQTT-02 | measured | HIGH | With `--shards >= 2`, MQTT delivery under sustained load is NON-DETERMINISTIC: subscribers on their own topics sometimes receive almost nothing. Two back-to-back runs of the SAME binary and config: `shards=2` gave 769 453 msg/s with 4 of 4 subscribers reporting, then 2 171 msg/s with 1 of 4 — the other three never received enough to finish a 5 s window. `shards=1` is stable (2.2-3.7M, always 4 of 4). Light load is fine (a paho probe gets 200/200 on 8 topics at s1 and s2), so it takes sustained load to appear | Not yet traced. It is not a throughput pathology — an earlier note in the benchmark file called it that and was wrong; a subscriber that receives ~nothing while its topic is being published to is a delivery defect. Needs the cross-shard fan-out instrumented (which shard holds the subscription, which holds the publish, where the delivery is dropped or starved) before any fix | measured 2026-09-03, `bench/mqttload.d` |
