...
| D-MQTT-02 | measured | HIGH | With `--shards >= 4`, MQTT loses deliveries under sustained load. Cleanest run: 4 independent publisher/subscriber pairs on their own topics, each publisher gets PUBACK for all 2 000 000 QoS-1 publishes, three subscribers receive 2 000 000 and the fourth receives **0** — same broker, same instant. Reproduced in every full-load s4 run (`delivered` = 1, 0, 194, 16 385, 64 062 across runs); `--shards 8` does not finish at all. Throttled to ~110k msg/s per pair, s4 still loses ~30 of 300 032 per pair (0.01%) — below the 396k msg/s per pair that s1 sustains losslessly. `--shards 1` and `--shards 2` are clean: 100% delivered in every controlled run. Low load is clean at s4 too (50k/pair: 12/12 pairs exact). Server CPU for the same work is ~26x at s4 (61s vs 2.35s wall-adjusted) | Not traced. **Open alternative, NOT eliminated**: the skin deliberately drops QoS-0 deliveries at a full per-subscriber outbox (`gMqttDropped`, spec-legal), and the subscriber here is QoS 0. Two attempts to rule it out both failed and neither should be cited: `$SYS/broker/messages/dropped` read 0, but the counter also read 0 for a subscriber that never read its socket at all, so it does not validate; and a QoS-1 measurement was invalid because the harness's own PUBACK path was broken (results were powers of two — an inflight stall in the client). Arguing against the policy: it is per-subscriber and rate-driven, yet s1 loses nothing at a HIGHER per-subscriber rate than the one at which s4 loses. Needs the cross-shard fan-out instrumented, and a working QoS-1 client, before a fix | measured 2026-09-03, `bench/mqttload.d` + `test/` harness |
| D-MQTT-02h | — | — | **Superseded evidence**: the first version of this row cited "769 453 msg/s with 4 of 4 reporting, then 2 171 msg/s with 1 of 4" and blamed `shards >= 2`. Both came from a harness with two defects and must not be reused: `mqttload sub` checked its duration deadline only AFTER a blocking `receive()`, so a starved subscriber hung and printed nothing (read downstream as "the broker delivered nothing"), and there was no barrier between SUBACK and the first publish, so a late subscriber's tiny count was scored as broker failure. Fixed in `bench/mqttload.d`: 100 ms recv timeout so deadlines are checked while idle, `READY` on stdout after the SUBACK so a harness can hold the publishers, drain-to-quiet instead of a wall window so no subscriber leaves while its publisher is still running, and a line printed even when the count is 0. With those, `shards = 2` is clean | — | — |

| D-KAFKA-03 | measured | LOW | A topic that has data but is NOT in the registry has its partition count derived by a walk that starts at partition 0 and stops at the first empty one, so a topic whose records all sit on a higher partition answers 0 and is treated as unknown. The Java producer's sticky partitioner routinely puts a small run entirely on one partition, so the sparse layout itself is ordinary — measured, 800 records landed wholly on partition 6 | Needs the registry to be missing while the data survives, which normal operation does not produce: Metadata registers a topic the first time it is named, and CreateTopics registers it explicitly. Downgraded from MED after tracing it: `kafka-topics.sh --describe` failing was NOT this bug — describe uses the ALL-TOPICS Metadata form, which lists from the registry by design, so an unregistered topic is invisible to it no matter what the walk returns. Two fixes were written and both reverted (see below) | repro in the row below |
| D-KAFKA-03r | — | — | **Reproduction**: `--kafka-db 5` (puts the Kafka keyspace in a SELECT-able db), create a topic with 8 partitions, produce 800 records with `kafka-producer-perf-test`, `DEL` the empty partition lists, `HDEL kafka.topics <topic>`, then consume. **Attempted fixes, both reverted**: (a) probe every partition when the contiguous walk finds nothing — up to KAFKA_MAX_PARTITIONS length probes, each possibly a cross-shard hop; (b) one variadic EXISTS per OWNING shard, since a partition's owner is computable and the batch routes on its first key — fixed the single-shard case, a consumer went from 0 to 800 records, but not the sharded one. A keyspace glob, which is the right shape, cannot be issued from the skin: `gKafkaExec` routes by `args[1]`, so a KEYS lands on whichever shard the PATTERN hashes to and sees only that keyspace (the client-facing KEYS does span shards — verified, 8 partitions across 4) | — | — |
