...
%% dreads laundering: a hanging case must cost 60s, not CT's 30min default —
%% hangs ARE an expected failure mode against a foreign broker.
...
all() ->
    %% dreads laundering: direct_connection_tests dropped — Erlang "direct"
    %% connections run INSIDE the broker VM (dist rpc), impossible against a
    %% non-Erlang broker. network_connection_tests is pure wire AMQP.
    [
    ...
    [
      ...
      {network_connection_tests, [], [
          {parallel_tests, [parallel], [
              basic_get_ipv4,
              basic_get_ipv6,
              %% basic_get_ipv4_ssl / basic_get_ipv6_ssl: DROPPED (dreads
              %% laundering) — need the CT-generated TLS certs + a TLS AMQP
              %% listener the external broker does not expose.
              pub_and_close,
              ...
              | ?COMMON_PARALLEL_TEST_CASES]},
          {non_parallel_tests, [],
              %% connection_blocked_network: DROPPED (dreads laundering) — it
              %% raises a resource alarm via `make set-resource-alarm` inside
              %% the broker node.
...
init_per_suite(Config) ->
    %% dreads laundering: the broker is EXTERNAL (dreads --amqp-port 5672);
    %% no node boot, no rabbitmqctl. Static config replaces the CT node config.
    dcfg(Config, [
        {rmq_hostname, "127.0.0.1"},
        {rmq_username, <<"guest">>},
        {rmq_password, <<"guest">>},
        {rmq_vhost, <<"/">>},
        {rmq_channel_max, 0},
        {rmq_unauthorized_username, <<"test_user_no_perms">>},
        {rmq_unauthorized_password, <<"test_user_no_perms">>}
    ]).
...
init_per_group(direct_connection_tests, Config) ->
    dcfg(Config, {amqp_client_conn_type, direct});
init_per_group(network_connection_tests, Config) ->
    dcfg(Config, {amqp_client_conn_type, network});
init_per_group(Group, Config)
  ...
  orelse Group =:= hard_error_loop ->
    case ?config(amqp_client_conn_type, Config) of
        undefined -> dcfg(Config, {amqp_client_conn_type, network});
        _         -> Config
    end.
...
init_per_testcase(Test, Config) ->
    ...
    {Port, SSLOpts} = if
        ...
        true ->
            {5672, none} %% dreads laundering: fixed external AMQP port
    end,
    ...
    dcfg(Config, {amqp_client_conn_params, ConnParams}).
...
%% dreads laundering: inline replacement for rabbit_ct_helpers:set_config/2.
dcfg(Config, KVs) when is_list(KVs) ->
    lists:foldl(fun(KV, C) -> dcfg(C, KV) end, Config, KVs);
dcfg(Config, {K, V}) ->
    lists:keystore(K, 1, Config, {K, V}).
