...
| unit/pubsub | 31 | 4 | 23→31, and the file now RUNS TO COMPLETION (was hanging). DONE: PING RESP2 `["pong",<arg>]`; CLIENT REPLY OFF/SKIP spares (un)subscribe confirmations; **XCLAIM full option parsing** (IDLE/TIME/RETRYCOUNT/FORCE/JUSTID/LASTID — the missing FORCE parse was the "Invalid stream ID" abort); **stream keyspace events** (xgroup-create/destroy/setid/delconsumer + xgroup-createconsumer for explicit XGROUP CREATECONSUMER *and* auto-creation in XREADGROUP`>`/XCLAIM/XAUTOCLAIM); **HMSET `hset`**; **EXPIRE-past-deadline fires `expired` not `del`**; **`new` keyevent** (`n` flag, central Keyspace.dbAdd hook). Remaining 4: (1) CONFIG GET canonical `notify-keyspace-events` flag string (A-collapse + order: KA→`AK`); (2-4) **publish-to-self inside MULTI / inside script / unsubscribe-in-MULTI** — a pubsub-in-transaction delivery cluster (a client subscribed to a channel it PUBLISHes to within MULTI/EVAL must still get its own message). |
