...
| unit/pubsub | ~23 | ~2 | `Pub/Sub PING on RESP2` FIXED (2-element `["pong",<arg>]` in RESP2 subscribe mode). CLIENT REPLY OFF/SKIP no longer suppresses (un)subscribe confirmations (they're protocol pushes the client blocks on — a REPLY-OFF that ate them HUNG the client). **Still open (real feature, own pass): keyspace notifications for the full HASH + STREAM command surface** — currently ~absent (hash 0 events, stream only `xsetid`). Needs event names (hset/hincrby/…, xadd/xgroup-*/xclaim/…), the empty-key→`del` secondary events, auto-consumer-creation events fired from inside XREADGROUP/XCLAIM, and a stream-command "Invalid stream ID" abort to root-cause. Do NOT cherry-pick just the test's commands. |
