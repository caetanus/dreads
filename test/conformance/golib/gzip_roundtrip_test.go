...
// Produce gzip-COMPRESSED records, then consume them back and verify the values
// survive dreads' decompress-on-ingest.
func TestGzipRoundTrip(t *testing.T) {
	...
	want := []string{}
	val := bytes.Repeat([]byte("compressible-payload-"), 30)
	for i := 0; i < 5; i++ {
	...
	for i := range want {
		if got[i] != want[i] { t.Fatalf("record %d mismatch: got %q want %q", i, got[i][:8], want[i][:8]) }
	}
	t.Logf("gzip round-trip OK: 5 compressed records produced + consumed, values intact")
}
